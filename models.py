...
class User(Base):
    ...
    mode       = Column(String(16),  default="manual")
...
class JournalEntry(Base):
    ...
    note        = Column(Text,       nullable=True)
    error_tag   = Column(String(32), nullable=True)
    created_at  = Column(DateTime,   default=datetime.utcnow)
...
class SmartWalletTx(Base):
    ...
    contract   = Column(String(64),  nullable=False)
    label      = Column(String(64),  nullable=True)
    action     = Column(String(8),   default="buy")
...
class SeenTx(Base):
    """Подпись транзакции — чтобы не обрабатывать дважды."""
    __tablename__ = "seen_txs"
    sig = Column(String(128), primary_key=True)


class FiredAlert(Base):
    """Дедупликация алертов по строковому ключу."""
    __tablename__ = "fired_alerts"
    # FIX: alert_key как primary key — s.get(FiredAlert, key) работает корректно
    alert_key  = Column(String(256), primary_key=True)
