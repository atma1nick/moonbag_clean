...
# FIX: используем alert_key как PK — s.get работает корректно
...
async def _process_personal_wallet(bot, user: User):
    ...
    for tx in txs[:5]:
        ...
        sol = _sol_spent(tx, user.wallet) or 0.5
        ...
        log.info(f"auto-track: {data['symbol']} → {user.user_id}")
